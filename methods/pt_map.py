...
from utils  import Logger, extract_features
...
class PT_MAP(object):
    ...
    def run_task(self, ndatas, labels, args=None):
        ...
        z_s = ndatas[:, :n_lsamples]
        z_q = ndatas[:, n_lsamples:]
        y_s = labels[:, :n_lsamples]
        y_q = labels[:, n_lsamples:]
        ...
        n_tasks = ndatas.size(0)
        if args.balanced == 'dirichlet':
            support = support.reshape(n_tasks, self.n_ways, self.n_shot, -1).permute(0,2,1,3).reshape(n_tasks,self.n_ways*self.n_shot,-1)
            y_s = y_s.reshape(n_tasks, self.n_ways, self.n_shot).permute(0,2,1).reshape(n_tasks,-1)

        data = torch.cat((support, query), dim=1)

        data = self.QRreduction(data)
        data = self.scaleEachUnitaryDatas(data)
        data = self.centerData(data, self.n_shot)
        ...
        gaus_model.initFromLabelledDatas(data=data, n_tasks=self.number_tasks,
                                         shot=self.n_shot, n_ways=self.n_ways, n_queries=self.n_queries, n_nfeat=data.size(2))

        self.run_adaptation(model=gaus_model, data=data, y_s=y_s, y_q=y_q, shot=self.n_shot)
