...
from extract_train_mean import extract_mean_features

# from image_demo.test_cifarfs import distribution
...
class BDCSPN(object):
    ...
    def proto_rectification(self, support, query, shot):
        ...
        if not self.balanced:
          support_ = support.reshape(support.shape[0], self.n_ways, shot, support.shape[-1]).mean(2)  # Init basic prototypes Pn
        else:
          support_ = support.reshape(support.shape[0], shot, self.n_ways, support.shape[-1]).mean(1)
    ...
    def run_task(self, ndatas, labels, args=None):
        ...
        z_s = ndatas[:, :n_lsamples].to(self.device)
        z_q = ndatas[:, n_lsamples:].to(self.device)
        y_s = labels[:, :n_lsamples].long().to(self.device)
        y_q = labels[:, n_lsamples:].long().to(self.device)

        train_mean, _ = extract_mean_features(args)
        ...
        # y_s = y_s.numpy()[:, :self.n_ways][0]
        y_s = np.arange(self.n_ways)
        y_q = y_q.cpu().numpy()

        # self.logger.info(" ==> Executing proto-rectification ...")
        support = self.proto_rectification(support=support, query=query, shot=self.shot)

        # Run adaptation
        self.run_prediction(support=support, query=query, y_s=y_s, y_q=y_q, shot=self.shot)
