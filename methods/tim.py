...
from utils import get_mi, get_cond_entropy, get_entropy, get_one_hot, Logger, extract_features
...
class TIM(object):
    ...
    def run_task(self, ndatas, labels, args=None):
        ...
        support = ndatas[:, :n_lsamples]
        query = ndatas[:, n_lsamples:]
        y_s = labels[:, :n_lsamples].long()
        y_q = labels[:, n_lsamples:].long()
        ...
        # Run adaptation
        self.run_adaptation(support=support, query=query, y_s=y_s, y_q=y_q, shot=self.shot)
