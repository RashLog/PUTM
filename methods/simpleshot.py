...
class SimpleShot(object):
    ...
    def run_task(self, ndatas, labels, args):
        ...
        if args.balanced == 'dirichlet':
          support = support.reshape(self.number_tasks, self.n_ways, self.shot, support.shape[-1]).mean(2)
        else:
          support = support.reshape(self.number_tasks, self.shot, self.n_ways, support.shape[-1]).mean(1)
        # Run adaptation
